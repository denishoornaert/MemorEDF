...
experiments of scale.}{Both cases have their own drawbacks. The former gives a
great deal of flexibility but has to abstract many part of the simulated SoC. In
addtion, simulated setups can hardly be applied on real platforms unless they rely on
soft-core implementation. The drawbacks posed by the full soft-core based SoC
implementation is that they (i) run at such a low frequency that they cannot
put enough pressure on the DRAM controller and (ii) rely on soft-cores that do not
exploit the same Instructions Set Architecture as widely available COTS. The last
point limits the practical impacts of these works.}
