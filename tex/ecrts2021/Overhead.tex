...
\new{ By design, the PLiM module proposed in this paper, the \schim,
  ...
  \fig{fig:schim_behaviour_tdma} highlights this problem. Even
  ...
  fact, if one assumes a PL frequency of 250~MHz and a single-burst
  ...
  3.7~GBps. This is in line with the throughput reported
  ...
  operates at 500~MHz whereas the CPUs operate at 1.5GHz. On the other
  ...
  (CCI+Main Interconnect, see \fig{fig:PS-PL-diagram}) and
...
%    By forwarding transactions
%    to the PL through the HPM, there is a drop in bandwidth as
%    accesses through the \emph{normal route} can extract up to
%    2100~MBps of bandwidth based on Figure \ref{fig:bandwidth_comparison}.
    Our experiments in Section \ref{subsec:platform-capabilities-and-performance-degradation}
    have shown that rerouting the traffic through the PL has a cost. As illustrated
    in \fig{fig:bandwidth_comparison}, up to 2100~MBps can be extracted
    from the \emph{normal route} whereas any route through the PL only achieves
    around 320~MBps.
    In contrast, a back-of-the-envelope calculation reveals that for a PL
    operating at 250~MHz (the \schim frequency), and with a bus width of 128 bits,
    a full-duplex throughput of approximately 3.7~GBps can be sustained.
%    This calculation is in line with the reported throughput in
%    an experiment conducted in \cite{uiuc-xilinx-port-study}, in which
%    transactions are initiated on the PL and target the DRAM, passing
%    through the HPS port.
    This calculation is in line with the reported throughput in
    an experiment conducted in \cite{uiuc-xilinx-port-study}, in which
    PL-originated transactions targeting the DRAM passed through the one of the
    HP ports.
%    By looking at this evidence, the PL-to-DRAM route
%    can sustain much higher throughput than what has been experimentally observed
%    with routing through the HPM port.
...
%    is left to be determined by further investigattion of the components
%    between LLC and HPM, such as throttling bandwidth regulators,
%    clock-domain crossing, and shallow FIFOs depicted in the platform
%    TRM \cite{Xilinx-ULTRASCALE-TRM} (Figure 1-1).
