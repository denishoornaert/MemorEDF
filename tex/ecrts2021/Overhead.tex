...
<<<<<<< HEAD
  platform rather than of the proposed design.}{By forwarding transactions
    to the PL through the HPM, there is a drop in bandwidth as
    accesses through the PS-to-DDR4 route can extract more than
    2100MBps of bandwidth based on Figure \ref{fig:bandwidth_comparison}.
    On the one hand, the back-of-the-envelope calculation stating
    that a PL operating at 250MHz with a single-burst beat of
    128-bits per clock cycle should be able to sustain a throughput of 3.7GB/s.
    Moreover, this calculation is in line with the reported throughput in
    an experiment conducted in \cite{uiuc-xilinx-port-study}, in which
    transactions were initiated on the PL and targetted the DRAM, passing
    through the HPS port. By looking at this evidence, the PL-to-DDR4 route
    can sustain much higher throughput than what experimentally observed
    with routing through the HPM port. Hence, identifying the real bottleneck
    ...
    TRM \cite{Xilinx-ULTRASCALE-TRM} (Figure 1-1). Finally, for the aformentioned reasons,
    the performance of the \schim is expected to vary between different platforms.}
...
>>>>>>> f0064932dee6f1e5534576323044b6ed751ce0f0
