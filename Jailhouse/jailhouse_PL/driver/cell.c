...
static int cell_switch_mapping(struct cell *cell, struct jailhouse_cell_mapping *mapping)
{
	int err;
	err = jailhouse_call_arg2(JAILHOUSE_HC_CELL_SWITCH_MAPPING, cell->id, mapping->mapping_id);
	if (err)
		return err;

	pr_info("Correctly switched mapping of cell %d to mapping %d\n",
		cell->id, mapping->mapping_id);
...
int jailhouse_cmd_cell_switch_mapping(const char __user *arg)
{
	struct jailhouse_cell_mapping mapping;
	struct cell *cell;
	int err;

	if (copy_from_user(&mapping, arg, sizeof(mapping)))
		return -EFAULT;

	err = cell_management_prologue(&(mapping.cell), &cell);
	if (err)
		return err;

	pr_info("Switching to mapping #%d for cell %d\n", mapping.mapping_id, cell->id);
	err = cell_switch_mapping(cell, &mapping);
