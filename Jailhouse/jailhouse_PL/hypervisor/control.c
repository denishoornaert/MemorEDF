...
enum management_task {CELL_START, CELL_SET_LOADABLE, CELL_DESTROY, CELL_SWITCH_MAPPING};
...
void refresh_root_page_cpu(unsigned int cpu_id)
{
	...
	target_data->refresh_root_page = true;
...
#if 0
void wait_for_cpu(unsigned int cpu_id)
{
	struct public_per_cpu *target_data = public_per_cpu(cpu_id);

	spin_lock(&target_data->control_lock);

	/* Empty critical section just to wait for previous operation completion. */
	spin_unlock(&target_data->control_lock);
}
#endif
...
static int cell_create(struct per_cpu *cpu_data, unsigned long config_address)
{
	...
	/* After this, we are ready to map-in all the memory
	   region. But firt we need to initialize the alternative
	   mappings by performing a copy of the current root_table
	   which contains important entries for MMIO setup. */
	arch_init_alt_mappings(cell);
#endif
...
static int cell_switch_mapping(struct per_cpu *cpu_data, unsigned long id,
			       unsigned long mapping_id)
{
	...
	err = cell_management_prologue(CELL_SWITCH_MAPPING, cpu_data, id, &cell);
	if (err)
		return err;
	
	// printk("Switching to mapping %ld for cell \"%s\"\n", mapping_id, cell->config->name);
	
	arch_cell_switch_mapping(cell, mapping_id);
...
long hypercall(unsigned long code, unsigned long arg1, unsigned long arg2)
{
	...
	case JAILHOUSE_HC_CELL_SWITCH_MAPPING:
		return cell_switch_mapping(cpu_data, arg1, arg2);
