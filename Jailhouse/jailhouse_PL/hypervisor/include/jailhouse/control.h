...
#define DROPLATOR_0_BASE     ((u64)0x1000000000)
#define DROPLATOR_0_RANGE    ((u64)0x0800000000) /* 32GB Range*/
#define DROPLATOR_0_CONFIG   ((u64)0x0080000000)
#define DROPLATOR_1_BASE     ((u64)0x4800000000)
#define DROPLATOR_1_RANGE    ((u64)0x0800000000) /* 32GB Range*/
#define DROPLATOR_1_CONFIG   ((u64)0x0090000000)
...
/**
 * Loop-generating macro for iterating over all the alternative
 * mappings for memory regions of a configuration.
 * @param mem		Iteration variable holding the reference to the current
 * 			alternative memory mapping (const struct jailhouse_memory *).
 * @param config	Cell or system configuration containing the regions.
 * @param counter	Helper variable (unsigned int).
 */
#define for_each_alt_mapping(mem, config, counter)			\
	for ((mem) = jailhouse_cell_alt_mappings(config), (counter) = 0; \
	     (counter) < (config)->num_alt_mappings;			\
	     (mem)++, (counter)++)
...
/**
 * Refresh the root page table of a remote CPU.
 * @param cpu_id	ID of the target CPU.
 *
 * @note This function must not be invoked for the caller's CPU.
 *
 * @see suspend_cpu
 */
void refresh_root_page_cpu(unsigned int cpu_id);
...
/**
 * Performs the architecture-specific steps to initialize the
 * alternative mappings, if any.  
 * @param cell Cell to initialize alternative mappings for.
 *
 * @see arch_cell_switch_mapping
 */
void arch_init_alt_mappings(struct cell *cell);

/**
 * Performs the architecture-specific steps for switching memory
 * mapping in a cell.  
 * @param cell Cell to switch root mapping for.
 * @param mapping_id ID of the new alternative mapping to install
 *
 * @see arch_cell_destroy
 */
...
/* Stuff to deal with droplator configuration */
