...
{
	return __paging_create_colored(pg_structs, phys, size, virt,
				       flags, col_val, coherent, remap_root,
				       NULL);
}


int __paging_create_colored(const struct paging_structures *pg_structs,
			    unsigned long phys, unsigned long size, unsigned long virt,
			    unsigned long flags, unsigned long col_val,
			    enum paging_coherent coherent, bool remap_root,
...
#if DEBUG_MAPPINGS
		if (printed == 0) {
			printk(" == [%s] Creating Colored Mapping == \n",
			       (remap_root ? "ROOT" : "CELL"));
		}
		
		if (printed++ < stopat) {
			printk("Page [%04x]: virt: 0x%08lx -> phys: 0x%08lx\n",
			       printed, virt, phys);
		}
