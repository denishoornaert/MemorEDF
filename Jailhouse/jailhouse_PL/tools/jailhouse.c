...
static int cell_simple_cmd(int argc, char *argv[], unsigned int command)
{
	...
	if (command == JAILHOUSE_CELL_ALT_MAPPING) {
		...
		mapping.mapping_id = strtol(argv[4], &endptr, 0);
		if (errno != 0 || *endptr != 0 || mapping.mapping_id < 0)
			help(argv[0], 1);

		fd = open_dev();

		err = ioctl(fd, command, &mapping);
	...
	if (err)
		perror(command == JAILHOUSE_CELL_START ?
		       ...
		       command == JAILHOUSE_CELL_ALT_MAPPING ?
		       "JAILHOUSE_CELL_ALT_MAPPING" :
		       "<unknown command>");
...
static int cell_management(int argc, char *argv[])
{
	...
	} else if (strcmp(argv[2], "alt") == 0) {
		err = cell_simple_cmd(argc, argv, JAILHOUSE_CELL_ALT_MAPPING);
	} else {
