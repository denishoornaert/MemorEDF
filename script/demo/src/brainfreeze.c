...
int main(int argc, char** argv) {
    ...
    (*config).deadlines[0] = thresholds;
    (*config).deadlines[1] = thresholds;
    (*config).deadlines[2] = thresholds;
    (*config).deadlines[3] = thresholds;
...
//        printf("Core %d started\n", 1);
        ...
        while(loop_activated) {
            for (unsigned i = ((1<<14)/sizeof(unsigned)); i < (HPM0_SIZE/sizeof(unsigned)); i+=(64*KB/sizeof(unsigned))) {
