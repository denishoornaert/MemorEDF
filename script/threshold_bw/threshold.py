...
def plot_fp():
    ...
    for core in range(1, 5, 1):
        ...
        axs[coord[core-1][0], coord[core-1][1]].set_ylim([0, 500])
        for prio in range(1, 5, 1):
            data = read("fp/prio_"+str(prio)+"/"+str(core)+"_cores.csv", 1)
...
def plot_ts_rev():
    ...
    for mit in range(1, 5, 1):
        ...
        for core in range(1, 5, 1):
            ...
            axs[coord[mit-1][0], coord[mit-1][1]].errorbar(x, y, yerr=e, label="Contention level: "+str(core)+" core"+("s" if (core > 1) else " (Solo)"))

            axs[coord[mit-1][0], coord[mit-1][1]].set_xticks(x)
            axs[coord[mit-1][0], coord[mit-1][1]].set_xticklabels([str(x[j]) for j in range(1,9)]+["Disabled"])
    ...
    fig.savefig("ts_rev.pdf", dpi=300)
