...
static void uart_init(struct uart_chip *chip)
{
	...
	mmio_write32(chip->virt_base + UT_READ, 0);
	mmio_write32(chip->virt_base + UT_WRITE, 0);
}

static bool uart_is_busy(struct uart_chip *chip)
{
	static unsigned long size = 0;
	unsigned long read =  mmio_read32(chip->virt_base + UT_READ);
	unsigned long write = mmio_read32(chip->virt_base + UT_WRITE);

	if (size == 0)
		size = chip->debug_console->size - UT_BUFSTART;
...
static void uart_write_char(struct uart_chip *chip, char c)
{
	...
	unsigned long write = mmio_read32(chip->virt_base + UT_WRITE);
	mmio_write8(chip->virt_base + UT_BUFSTART + write, c);
	mmio_write32(chip->virt_base + UT_WRITE, write + 1);
}
